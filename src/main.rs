...
fn nginx_handler<R: Request>(mut req: R) -> anyhow::Result<()> {
...
impl<R, W> MockRequest<R, W> {
    fn new(reader: R, writer: W, range: impl RangeBounds<u64>) -> Self {
        Self {
            reader,
            writer,
...
fn main() -> anyhow::Result<()> {
    let car_file = File::open("QmT78zSuBmuS4z925WZfrqQ1qHaJ56DQaTfyMUF7F8ff5o.car")?;
    let car_reader = BufReader::new(car_file);

    let mut buff = io::Cursor::new(vec![0; 112]);

    let req = MockRequest::new(car_reader, &mut buff, 0..50);
